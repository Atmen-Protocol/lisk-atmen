...
class Point {
	...
	static fromPrivateKey(k: PrivKey) {
...
const n2b = (num: bigint): Bytes => {
	// number to 32bytes. mustbe 0 <= num < B256
...
function getPublicKey(privKey: PrivKey, isCompressed = true) {
...
function prepSig(msgh: Hex, priv: Hex, opts = optS): BC {
	// prepare for RFC6979 sig generation
...
function hmacDrbg<T>(asynchronous: boolean) {
	// HMAC-DRBG async
	...
	if (asynchronous) {
		...
		return async (seed: Bytes, pred: Pred<T>): Promise<T> => {
			// Even though it feels safe to reuse
		...
		const h = (...b: Bytes[]) => {
			// asynchronous=false; same, but synchronous
			const f = _hmacSync;
...
function hashToPrivateKey(hash: Hex): Bytes {
...
export {
	...
	signAsync,
	verify,
	CURVE, // Remove the export to easily use in REPL
	getSharedSecret,
	etc,
	utils,
